...
## 8. プロセス管理とUI整形の高度化 (NEW)

### 確実な中断処理 (Process Group Termination)
外部CLIから呼び出される子プロセス（さらにその先の子プロセス）を確実に停止させるため、プロセスグループ単位での管理を導入しました。
- `start_new_session=True`: 子プロセスを新しいセッション（PGID）で起動します。
- `SIGTERM` -> 250ms待機 -> `SIGKILL`: Node.jsアプリがクリーンアップ（終了イベントの送出）を行うための猶予を与えた後、強制終了します。
- `os.killpg(pgid, signal)`: これにより、シェル経由で起動された孫プロセスも含めて一掃し、ゾンビプロセスを防ぎます。

### クライアントサイドでの再帰的JSON整形
エージェントによるログ解析（生データが必要）と、人間によるモニター監視（読みやすさが必要）を両立させるため、整形ロジックを全面的にクライアント（ブラウザ）へ移行しました。
- **再帰的意味抽出エンジン**: Claudeなどの複雑なネスト構造（リスト形式の `content` 等）を自動で探索し、テキストや思考プロセスを抽出します。これにより `[object Object]` の表示を根絶しました。
- **生データの維持**: サーバーから送出される通知（Notification）は一切加工せず、生のJSONプロトコルを維持します。
- **二重プレフィックス防止**: 抽出済みテキスト内の絵文字を検出し、重複するアイコン付与を防止します。
- **連結JSONの処理**: `stdout` から流くる `}{` のような連結されたJSONオブジェクトを、パース前に `}\n{` へ置換して分割処理します。
- **ブラウザでの実機確認の徹底**: `monitor/dashboard.html` を修正した際は、ブラウザ上での JavaScript 構文エラーや表示の崩れを防ぐため、エージェントは必ず `take_snapshot` や `list_console_messages` ツールを使用して、ブラウザ実機での動作確認（およびコンソールエラーのチェック）を行う必要があります。

### トークン計算とメトリクスの正確性
- **Thinking Tokenの分類**: GeminiやClaudeの思考トークンは、プロバイダーの仕様に基づき「出力（Output）トークン」として集計・課金対象として扱います。
- **ツール実行時間の正確性**: ストリーミングイベント（`content_block_start`）のタイムスタンプを始点として記録し、後続の `user` メッセージ（ツール結果）で確定させることで、ミリ秒単位の正確な実行時間を算出します。
