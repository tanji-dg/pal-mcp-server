                ...
                for line in lines:
                    ...
                    def process_obj(obj, current_sid=None):
                        ...
                        if new_model and isinstance(new_model, str):
                            ...
                            high_level_identifiers = ["claude", "codex", "gemini", "sonnet", "haiku", "opus", "gpt-4", "o1", "o3", "flash"]
                            is_current_generic = not self.model_name or any(self.model_name.lower() == h for h in high_level_identifiers)
                            is_new_generic = any(new_model.lower() == h for h in high_level_identifiers)
                            
                            # Decision logic:
                            # 1. If we have nothing, take it.
                            # 2. If current is generic and new is detailed, take it.
                            # 3. If new is different and not generic, and current is generic, take it.
                            # 4. If new is detailed and current is detailed but different, take it (Model switch).
                            if not self.model_name:
                                self.model_name = new_model
                            elif is_current_generic and not is_new_generic:
                                logger.info(f"Model Name Update (Generic -> Detailed): {self.model_name} -> {new_model}")
                                self.model_name = new_model
                            elif not is_new_generic and self.model_name != new_model:
                                # Switch between different detailed models (e.g. pro -> flash in sub-calls)
                                logger.info(f"Model Name Update (Switch): {self.model_name} -> {new_model}")
                                self.model_name = new_model
